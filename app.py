...
if has_status:
    section_header("공정 현황 요약", "중층과 표층의 진행률, 잔여 물량, 생산성, 예상 완료일을 요약합니다.", "status-summary")
    ...
    with st.container(border=True):
        ...
        if not summary_df.empty:
            surface_summary = summary_df[
                summary_df["단위"].astype(str).str.contains("㎡", na=False)
            ].copy()

            middle_summary = summary_df[
                ~summary_df["단위"].astype(str).str.contains("㎡", na=False)
            ].copy()

            surface_design = surface_summary["설계수량"].sum() if not surface_summary.empty else 0
            surface_done = surface_summary["누계"].sum() if not surface_summary.empty else 0

            middle_design = middle_summary["설계수량"].sum() if not middle_summary.empty else 0
            middle_done = middle_summary["누계"].sum() if not middle_summary.empty else 0

            surface_progress = surface_done / surface_design * 100 if surface_design > 0 else 0
            middle_progress = middle_done / middle_design * 100 if middle_design > 0 else 0
        ...
        with row1_col2:
            st.markdown(f'<div class="metric-card"><div class="metric-title">중층 진행률</div><div class="metric-value">{middle_progress:.1f}%</div></div>', unsafe_allow_html=True)
        with row1_col3:
            st.markdown(f'<div class="metric-card"><div class="metric-title">표층 진행률</div><div class="metric-value">{surface_progress:.1f}%</div></div>', unsafe_allow_html=True)
        ...
        with row2_col2:
            st.markdown(f'<div class="metric-card"><div class="metric-title">중층 최근 평균</div><div class="metric-value">{middle_recent_avg_text}</div></div>', unsafe_allow_html=True)
        with row2_col3:
